...
- 直接推荐人获得下级质押数量 5% 的 SHD，一次性释放；该奖励由本笔质押转入合约后的余额直接支付
- 下级用户提前结算时，静态收益全无，并从该订单本金中追扣本单产生的直推 5%
...
- `batchSetReferrers(referrers, users)` 管理员后台按“邀请人,被邀请人”导入推荐关系
- `purchasePackage(packageAmount, orderRef)` 产品包扣款进 DApp 合约资金池
- `fundRewards(amount)` 管理员授权转账充入合约余额
- `withdrawShd(to, amount)` 管理员提取合约余额

合约资金模式：

- DApp 合约自身持有 SHD
- 用户认购/产品包购买会把 SHD 转入 DApp 合约余额
- 普通质押产生的 5% 直推奖励从该笔质押转入后的合约余额直接支付，不要求项目方提前预存奖励池
- 到期静态收益、团队补贴领取、管理员手工建仓等后续支付，仍以 DApp 合约当时实际 SHD 余额为准
- 管理员可以通过 `fundRewards` 充入合约余额，也可以直接向合约地址转账 SHD
...
- 管理后台只保留 DApp 合约信息、合约余额管理、用户关系导入
...
前端通过 `NEXT_PUBLIC_APP_MODE` 选择目标链：

- `development`：本地 Hardhat 链，Chain ID `31337`，RPC `http://127.0.0.1:8545`
- `production`：正式 DOR Network
...
```bash
NEXT_PUBLIC_APP_MODE=development # development | production
NEXT_PUBLIC_DAPP_ADDRESS=0xYourDeployedSHDStakingAddress
```

`pnpm dev:testnet` 会自动部署本地合约并写入 `.env.development`：

```bash
NEXT_PUBLIC_APP_MODE=development
NEXT_PUBLIC_DAPP_ADDRESS=0xLocalSHDStakingAddress
```

`pnpm deploy:prod` 会部署正式合约并写入 `.env.production`：

```bash
NEXT_PUBLIC_APP_MODE=production
NEXT_PUBLIC_DAPP_ADDRESS=0xProductionSHDStakingAddress
```
